...
class Lyrics:
    @staticmethod
    async def get(query: str) -> Union[dict, None]:
        """Fetches lyrics for the given query via Lrclib.net."""
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    "https://lrclib.net/api/search",
                    params={"q": query},
                    headers={"User-Agent": "SonicSearchAPI/2.1.0"},
                    timeout=10,
                )
                ...
                # Return the first result that actually has text content
...
class Recommendations:
    ...
    async def get_by_genre(genre: str, limit: int = 10) -> Union[dict, list, None]:
        ...
        api_key = Recommendations._get_api_key()
        if not api_key:
            return {"error": "LASTFM_API_KEY environment variable is missing."}
    ...
    @staticmethod
    async def get_similar_tracks(query: str, limit: int = 10) -> Union[dict, list, None]:
        """Search for a song and get similar track recommendations."""
        api_key = Recommendations._get_api_key()
        if not api_key:
            return {"error": "LASTFM_API_KEY environment variable is missing."}

        async with httpx.AsyncClient() as client:
            # 1. Search for the track to resolve artist and correct track name
            search_params = {"method": "track.search", "track": query, "api_key": api_key, "format": "json", "limit": 1}
            search_res = await client.get(Recommendations.BASE_URL, params=search_params)
            
            if search_res.status_code == 200:
                results = search_res.json().get("results", {}).get("trackmatches", {}).get("track", [])
                if results:
